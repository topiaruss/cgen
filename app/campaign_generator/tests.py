...
from .translation_service import TranslationService, MockTranslationProvider
...
def test_json_upload_form_valid():
    ...
    json_data = {
        "title": "Test Campaign",
        "products": [{"name": "Product A", "type": "Energy Drink"}, {"name": "Product B", "type": "Energy Drink"}],
        "target_region": "California",
        "target_audience": "Young adults",
        "campaign_message": "Test message",
    }
...
def test_generation_run_with_assets(brief):
    ...
    from .models import GenerationRun, GeneratedAsset
...
def test_billing_error_handling(mock_settings, campaign_generator):
    ...
    with mock_settings(AI_DEV_MODE=False):
        ...
        with patch.object(generator.client.images, 'generate') as mock_generate:
            mock_generate.side_effect = Exception("Error code: 400 - {'error': {'message': 'Billing hard limit has been reached', 'type': 'image_generation_user_error', 'param': None, 'code': 'billing_hard_limit_reached'}}")
...
def test_quota_error_handling(mock_settings, campaign_generator):
    ...
    with mock_settings(AI_DEV_MODE=False):
        ...
        with patch.object(generator.client.images, 'generate') as mock_generate:
            mock_generate.side_effect = Exception("Error code: 429 - {'error': {'message': 'You exceeded your current quota', 'type': 'insufficient_quota'}}")
...
def test_cost_estimation_in_views(mock_settings, client, brief):
    ...
    with mock_settings(AI_DEV_MODE=False):
        ...
        with patch('campaign_generator.ai_service.CampaignGenerator._call_dalle') as mock_dalle, \
             patch('campaign_generator.ai_service.CampaignGenerator._outpaint_landscape') as mock_landscape, \
             patch('campaign_generator.ai_service.CampaignGenerator._outpaint_vertical') as mock_vertical:
            ...
            response = client.post(
                reverse("generate_assets", kwargs={"brief_id": brief.id}),
                data={},
                follow=True
            )
...
@pytest.mark.django_db
def test_generated_asset_translation_relationship(generation_run, english_language, spanish_language):
...
def test_translation_service_translate_campaign_content():
    ...
    for key, original_text in content.items():
        ...
        # Accept either real translation or mock format
        assert (translated_text != original_text or 
                translated_text.startswith("[ES]") or
                "campaña" in translated_text.lower() or
                "energía" in translated_text.lower())
...
def test_prepare_example_data_returns_dict(load_languages):
    ...
    required_keys = [
        'title', 'target_region', 'campaign_message', 'primary_language',
        'products_json', 'language_names', 'language_ids', 'language_codes_csv',
        'target_audience', 'tip_text'
    ]
...
def test_prepare_example_data_static_content(load_languages):
    ...
    assert example_data['campaign_message'] == 'Natural energy that connects you to the coastal lifestyle'
...
def test_gallery_view_language_group_structure(load_languages, client):
    ...
    for group in language_groups:
        # Required keys
        required_keys = ['lang_name', 'lang_code', 'assets', 'count', 'lang_rtl', 'lang_ttb', 'lang_ltr']
        for key in required_keys:
...
def test_reference_image_normalization():
    ...
    test_file = SimpleUploadedFile("test.jpg", test_image_buffer.getvalue(), content_type="image/jpeg")
...
def test_brief_form_with_reference_image(load_languages):
    ...
    test_image = SimpleUploadedFile("reference.jpg", test_image_buffer.getvalue(), content_type="image/jpeg")
...
def test_json_upload_form_with_reference_image(load_languages):
    ...
    test_image = SimpleUploadedFile("reference.png", test_image_buffer.getvalue(), content_type="image/png")
    
    form_data = {}
    file_data = {
        'brief_file': json_file,
        'reference_image': test_image
    }
...
def test_reference_image_asset_generation(load_languages, brief_with_reference_image):
    ...
    with patch.object(CampaignGenerator, '_generate_assets_with_outpainting') as mock_outpaint:
        ...
        assert ref_asset.generation_time_seconds == 0.0, "Reference assets should have 0 generation time"
        ...
        expected_count = len(brief_with_reference_image.products) * len(brief_with_reference_image.get_all_languages()) * 3  # 3 aspect ratios
        assert len(reference_assets) == expected_count, f"Should create {expected_count} reference assets"


@pytest.mark.django_db 
def test_generated_asset_reference_metadata(load_languages):
    """Test GeneratedAsset reference image metadata fields"""
    from .models import GeneratedAsset, Brief, Language, GenerationRun
...
def test_create_brief_view_with_reference_image(client, load_languages):
    ...
    test_image = SimpleUploadedFile("test.jpg", test_image_buffer.getvalue(), content_type="image/jpeg")
...
def test_upload_brief_view_with_reference_image(client, load_languages):
    ...
    test_image = SimpleUploadedFile("ref.jpg", test_image_buffer.getvalue(), content_type="image/jpeg")
