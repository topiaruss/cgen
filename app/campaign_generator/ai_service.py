...
class CampaignGenerator:
    ...
    def _generate_assets_with_outpainting_in_memory(self, product, brief):
        ...
        landscape_response = self.client.images.generate(
            ...
            size="1024x1024",  # DALL-E 3 only supports square, we'll crop later
            quality="standard",
            n=1,
        )
        generation_time = time.time() - start_time
    ...
    def _generate_square_image_in_memory(self, product, brief):
        """IN-MEMORY: 1:1 - Square crop of core scene for Instagram feed"""
        core_prompt = self._build_core_scene_prompt(product, brief)
        aspect_prompt = f"""
        {core_prompt}
        Composed for square format (1:1). Center the product prominently.
        Frame tightly for social media feed engagement.
    ...
    def _generate_story_image_in_memory(self, product, brief):
        """IN-MEMORY: 9:16 - Vertical story format for Instagram/Snapchat"""
        core_prompt = self._build_core_scene_prompt(product, brief)
        aspect_prompt = f"""
        {core_prompt}
        Composed for vertical story format (9:16). 
    ...
    def _generate_landscape_image_in_memory(self, product, brief):
        """IN-MEMORY: 16:9 - Horizontal format for YouTube/Facebook"""
        core_prompt = self._build_core_scene_prompt(product, brief)
        aspect_prompt = f"""
        {core_prompt}
        Composed for horizontal format (16:9). 
